...
const base_1 = require("ionic/commands/ssh/base");
...
Before making changes, ${chalk_1.default.green('ionic ssh use')} will print a diff and ask for permission to write the file.
    ...
    run(inputs, options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            const { ERROR_SSH_INVALID_PRIVKEY, ERROR_SSH_MISSING_PRIVKEY, validatePrivateKey } = yield Promise.resolve().then(() => require('ionic/lib/ssh'));
            const { ensureHostAndKeyPath, getConfigPath } = yield Promise.resolve().then(() => require('ionic/lib/ssh-config'));
            ...
            const { SSHConfig } = yield Promise.resolve().then(() => require('ionic/lib/ssh-config'));
            ...
            if (text1 === text2) {
                ...
                const { diffPatch } = yield Promise.resolve().then(() => require('ionic/lib/diff'));
