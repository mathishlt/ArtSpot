...
const http_1 = require("ionic/lib/http");
const http_2 = require("ionic/lib/utils/http");
...
function getImageResources(projectDir) {
    const images = [];
    for (const platform in exports.RESOURCES) {
        ...
        for (const imageType in platformConfig) {
            ...
            for (const image of imageTypeConfig.images) {
                images.push({
                    platform,
                    resType: imageType,
                    dest: path.resolve(projectDir, 'resources', platform, imageType, image.name),
                    name: image.name,
                    width: image.width,
                    height: image.height,
 ...
 */
function getSourceImages(projectDir, buildPlatforms, resourceTypes) {
    return tslib_1.__awaiter(this, void 0, void 0, function* () {
        ...
        for (const srcImgDir of srcDirList) {
            const srcImageDirContents = yield utils_fs_1.readdirSafe(srcImgDir.path);
        ...
        const sourceImageChecksums = yield Promise.all(sourceImages.map((img) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            const [md5, cachedMd5] = yield utils_fs_1.getFileChecksums(img.path);
            if (cachedMd5) {
                debug(`${chalk_1.default.cyan('getSourceImages')}: ${chalk_1.default.bold(format_1.prettyPath(img.path))} cache md5: ${chalk_1.default.bold(cachedMd5)}`);
 ...
 */
function findMostSpecificSourceImage(imageResource, srcImagesAvailable) {
